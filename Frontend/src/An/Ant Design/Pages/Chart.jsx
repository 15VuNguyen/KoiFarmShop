...
function ChartCard({ title, data, dateField, chartType, onChartTypeChange, onDateRangeChange }) {
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetchData().then(({ accounts, orders, invoices, consigns }) => {
      ...
      const consignData = sortedConsigns.reduce(howManyStateCorpondeToEachDATES, {});
  ...
  const handleDateRangeChange = (type, value) => {
    ...
    if (type === 'accounts' && rawAccountData) {
      ...
      const filteredOrderData = filterAndAggregateData(rawOrderData, 'OrderDate');
      setOrderData({
      ...
      const filteredInvoiceData = filterAndAggregateData(rawInvoiceData, 'InvoiceDate');
      setInvoiceData({
    ...
    <Layout>
      ...
      <Flex justify='space-around' wrap='wrap'>
        <ChartCard
          ...
          onDateRangeChange={(value) => handleDateRangeChange('accounts', value)}
        />
        <ChartCard
          ...
          >
            ...
            >
              ...
              <Space direction="horizontal" style={{ marginBottom: 16 }}>
                <Radio.Group
                  defaultValue="year"
                  onChange={(e) => handleDateRangeChange('consigns', e.target.value)}
                >
                  <Radio.Button value="7days">7 Ngày Qua</Radio.Button>
                  <Radio.Button value="month">Tháng Trước</Radio.Button>
                  <Radio.Button value="year">Năm Trước</Radio.Button>
                ...
                <Tooltip title="Chuyển Đổi Loại Biểu Đồ">
                  <Switch
                    ...
                    onChange={() => setConsignChartType((prev) => (prev === 'bar' ? 'line' : 'bar'))}
                    checkedChildren={<BarChartOutlined />}
                    unCheckedChildren={<LineChartOutlined />}
                  />
                </Tooltip>
