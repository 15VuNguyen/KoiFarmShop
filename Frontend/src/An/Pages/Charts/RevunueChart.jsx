  ...
  const filterDataByDateRange = (data) => {
    ...
    if (dateRange === "7d") {
      ...
      filteredData = data.filter(
        (item) => item.Date && new Date(item.Date) >= sevenDaysAgo
      );
    } else if (dateRange === "month") {
      const lastMonth = new Date(today.setMonth(today.getMonth() - 1));
      filteredData = data.filter(
        (item) => item.Date && new Date(item.Date) >= lastMonth
      );
    } else if (dateRange === "year") {
      const lastYear = new Date(today.setFullYear(today.getFullYear() - 1));
      filteredData = data.filter(
        (item) => item.Date && new Date(item.Date) >= lastYear
      );
    }
            ...
            plugins: {
              tooltip: {
                callbacks: {
                  label: function (tooltipItem) {
                    const revenue =
                      filteredRevenueData[tooltipItem.dataIndex]?.TotalPrice ||
                      0;
                    const profit =
                      filteredProfitData[tooltipItem.dataIndex]?.TotalProfit ||
                      0;
                    const percentageChange = revenue
                      ? ((profit - revenue) / revenue) * 100
                      : 0;
                    ...
                    return `${
                      ...
                      "en-US"
                    )} đ (${percentageChange.toFixed(2)}% ${changeLabel})`;
                  },
                  labelTextColor: function (tooltipItem) {
                    const revenue =
                      filteredRevenueData[tooltipItem.dataIndex]?.TotalPrice ||
